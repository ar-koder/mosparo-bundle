...
class IsValidMosparoValidatorTest extends ConstraintValidatorTestCase
{
    ...
    protected function makeValidator(bool $submittable = false,
        bool $valid = false,
        ...
        $request->method('get')
            ->willReturnCallback(function ($query) use ($submitToken, $validationToken) {
                return match ($query) {
        ...
        $client->method('verifySubmission')->willReturn(new VerificationResult(
            $submittable,
            $valid,
            $verifiedFields,
            $issues
    ...
    protected function setValidator(bool $submittable = false,
        bool $valid = false,
        array $verifiedFields = [],
        array $issues = [],
        ?string $submitToken = self::SUBMIT_TOKEN,
        ?string $validationToken = self::VALIDATION_TOKEN): void
    {
